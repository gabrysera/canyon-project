...
class Pillar(object):
    ...
    def set_reachable_node(self, p, possible_disks): #this function can be used to add a reachable pillar object p to the dictionary of nodes accesible to the main pillar object, and the distance associated to reach it.
...
def possible_disks(dist, disks_pairs): #in this function, we include all the possible disc sizes that culd be used to reach the distance required to the other pillar, and in the end return the size of the cheapest disc.
    possible_disks = []
...
def reachable_pillars(p, pillars, threshold, disks_pairs): #given a starting pillar, it compares with every other pillar, and if the distance between them is less than the biggest sized disc available(threshold), then its added as a reachable node to both 
    for p0 in pillars:
        dist = distance(p0, p)
        if dist <= threshold:
...
def create_adjacency_matrix(W, pillars, disks_pairs, max_r): #this function creates the adjacency list by using dictionaries with each pillar object as key and its values including a nested dictionary of the nodes accessible to it and the cost to access it.
    start_index = 0
    for p in pillars:
        start_index += 1
        #reachable = 
